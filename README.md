...
**Ключи пользователей:** Каждый пользователь добавляет свои API ключи через настройки (⚡ в шапке). Система использует **личные ключи пользователя** для генерации ответов и эмбеддингов — каждый платит за свой использование. Системные ключи используются только как fallback для пользователей без ключей.
...
Knowledge Graph — граф знаний системы, хранящий концепции и связи между ними. Используется для:

- **Graph RAG** — контекст из графа инжектится в промпт LLM при каждом ответе (Phase 1)
- **Авто-экстракция** — каждое сообщение пользователя пополняет граф концепциями (Phase 2)
- **Синхронизация** — двусторонняя синхронизация SQLite (локально) ↔ Supabase (продакшн) (Phase 3)
- **Семантический поиск** — векторные эмбеддинги (OpenRouter text-embedding-3-small, 384 dim) + cosine similarity (Phase 4)
- **UI редактор** — добавление/удаление/мерж концепций, редактирование связей (Phase 5)

**Эмбеддинги пользователей:** Генерация эмбеддингов (POST `/recompute-embeddings`) использует **личный OpenRouter ключ пользователя** из его сохранённых провайдеров. Каждый пользователь платит за свои токены. Если ключа нет — возвращается ошибка с предложением добавить ключ в настройках (⚡).

**API endpoints:**
- `GET /api/v1/knowledge/search?q=...` — поиск по имени
- `GET /api/v1/knowledge/semantic-search?q=...` — семантический поиск (cosine similarity)
- `POST /api/v1/knowledge/recompute-embeddings?limit=50` — перегенерация эмбеддингов (использует ключ пользователя)
- `POST /api/v1/knowledge/concepts/{id}/merge` — мерж двух концепций
- `DELETE /api/v1/knowledge/concepts/{id}` — удаление концепции
- `PATCH /api/v1/knowledge/relations` — создание/обновление связи

---

## Research Platform

Комплексный инструментарий для исследования, отладки и анализа поведения когнитивной архитектуры PAD+ AI. Доступен на странице `#research` (Hash-навигация фронтенда).
...
Neural Link UI — визуальная схема всех фаз pipeline в реальном времени. Каждая фаза показывает:
- **Иконку категории** (cognition / memory / emotion / safety / generation / evaluation / output)
- **Статус** (pending / running / done / error)
- **Evaluation + Why-card** — раскрывающаяся карточка «почему такое решение» с деталями оценки (score, passed, метрики)
- **Длительность** выполнения фазы

**Live-обновления:** Microscope слушает WebSocket `/api/v1/xray/ws`. При разрыве соединения — автоматический fallback на `fetch` (polling).

### Replay Mode

Перепроигрывание сохранённых трасс для анализа поведения системы в прошлых сессиях:
- `POST /api/v1/experiments/replay` — загрузка сохранённой трассы в Microscope
- Снэпшоты доступны в `backend/experiments/snapshots/` (создаются автоматически и вручную)

### Compare Providers

Сравнение поведения LLM-провайдеров (OpenRouter / GigaChat) по ключевым метрикам:
- **Latency** — время генерации
- **Cost** — стоимость токенов
- **Quality** — оценка качества (через Evaluation pipeline)
- **Export** — выгрузка таблицы сравнения в **JSON** и **CSV** (`ProvidersCompareTab.jsx`)

### Decision Log ↔ Living Anatomy

Кросс-интеграция с модулем 🧬 Living Anatomy:
- Кнопка «Решения модуля» на странице Anatomy → переход на `#research?component=<component>`
- Фильтрует Decision Log по компоненту (источник: `decisions_routes.py` + `MODULE_TO_COMPONENT` в `backend/core/anatomy.py`)

### Snapshot ↔ Living Anatomy

- `#anatomy?snapshot=<id>` показывает срез снэпшота из Research на дереве модулей Anatomy
- Снэпшоты агрегируют состояние всех подсистем на момент времени

### Трассировка и очистка (Trace cleanup)

- `backend/core/xray/history_recorder.py` автоматически чистит трассы старше `TRACE_RETENTION_DAYS=30` (метод `cleanup_old()`)
- Seed experiments (`I-*`) создаются в `backend/experiments/runs/` при первом старте (если директория пуста)

### API endpoints (Research Platform)

| Эндпоинт | Назначение |
|---|---|
| `GET /api/v1/xray/current` | Текущая трасса (агрегирует все фазы + evaluation из `explanation.evaluation`) |
| `WS /api/v1/xray/ws` | WebSocket live-обновления трассировки |
| `GET /api/v1/experiments/runs` | Список прогонов / экспериментов |
| `POST /api/v1/experiments/replay` | Replay сохранённой трассы |
| `GET /api/v1/experiments/compare-providers` | Данные сравнения провайдеров |
| `GET /api/v1/decisions?component=<c>` | Decision Log, фильтр по компоненту (для Anatomy) |
| `GET /api/v1/anatomy/status` | Live-статус модулей (для Snapshot) |
...
│   └── src/
│       ├── pages/              # 15 страниц (Chat, Research, Anatomy, X-Ray и др.)
...
### Исследовательский интерфейс и Microscope
Новая итерация делает поведение системы заметным и понятным: пользователь может видеть, как формируется ответ, какие фазы pipeline выполняются, какие доказательства и оценки используются.

Ключевые добавления **Research Platform** (v4.1):
- **🔬 AI Under Microscope** — просмотр всех фаз pipeline в реальном времени на одной схеме (Neural Link UI)
- **Live X-Ray с WebSocket** — мгновенные обновления трассировки через `/api/v1/xray/ws` (fallback на polling при разрыве)
- **Replay Mode** — перепроигрывание сохранённых трасс через `POST /api/v1/experiments/replay`
- **Evaluation + Why-card** — для каждого шага: score/passed и раскрывающаяся карточка «почему такое решение» с деталями evaluation
- **Compare Providers** — сравнение OpenRouter/GigaChat (latency, cost, quality) с экспортом в JSON и CSV
- **Decision Log ↔ Living Anatomy** — кнопка «Решения модуля» на Anatomy → `#research?component=<component>`
- **Snapshot ↔ Living Anatomy** — `#anatomy?snapshot=<id>` показывает срез снэпшота из Research
- **Trace cleanup** — `history_recorder.py` чистит трассы старше `TRACE_RETENTION_DAYS=30`
- **Seed experiments** — при старте создаются демо-эксперименты (`I-*`) в `backend/experiments/runs/`

![Исследовательский интерфейс](screenshots/chat.png)

### Live X-Ray и наблюдаемость
Система теперь лучше показывает состояние обработки в реальном времени: доступен живой просмотр трассировки, этапов работы и ключевых сигналов, связанных с качеством ответа.

![X-Ray наблюдаемость](screenshots/xray.png)

### Провайдеры, сравнение и управление
Добавлены улучшения для работы с провайдерами, сравнения поведения моделей и более удобного управления подключениями.

![Настройка провайдеров](screenshots/providers.png)

### Навигация по знаниям и состояниям
Интерфейс стал более связанным: пользователь может быстрее переходить между знаниями, памятью, опытом и системой инструкций.

![Навигация по знаниям](screenshots/knowledge.png)

> Эти разделы можно легко обновлять по мере появления новых правок и новых экранов интерфейса.
...
| `screenshots/research_microscope.png` | 🔬 AI Under Microscope — все фазы pipeline в реальном времени (Neural Link UI) |
