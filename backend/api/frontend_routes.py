...
async def chat(
    ...
    if not api_key:
        ...
        logger.error(f"? No API key found for user {user_id}. key_id={request.key_id}, provider={request.provider}")
        raise HTTPException(
            status_code=400,
    ...
    try:
            ...
            try:
                ...
                if dialog_id:
                    svc = get_supabase_service()
                    # Извлекаем usage из результата пайплайна
                    usage = {}
                    if hasattr(result, 'llm_metadata') and result.llm_metadata:
                        usage = result.llm_metadata.get('usage', {})
                    elif hasattr(result, 'raw_llm_response') and result.raw_llm_response:
                        usage = result.raw_llm_response.get('usage', {})
                    
                    # Вычисляем стоимость (ориентировочные цены за 1K токенов)
                    cost = 0.0
                    if usage:
                        # Цены для OpenRouter (примерные, обновляются)
                        prompt_tokens = usage.get('prompt_tokens', 0)
                        completion_tokens = usage.get('completion_tokens', 0)
                        # ~$0.001 за 1K input + $0.002 за 1K output (средние цены)
                        cost = round((prompt_tokens * 0.001 + completion_tokens * 0.002) / 1000, 6)
                        usage['cost_usd'] = cost
