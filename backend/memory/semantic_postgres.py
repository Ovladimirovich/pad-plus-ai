...
class SemanticMemory:
    ...
    def _raw_get_conn(self):
        """Получает соединение из пула БЕЗ проверки доступности (для инициализации)."""
        from core.pg_pool import get_connection
        return get_connection()

    def _get_conn(self):
        """Получает соединение из пула с ленивой инициализацией и повторными попытками."""
        if self._available is None:
            # Попытки инициализации с повторами
            while self._init_attempts < self._max_init_attempts:
                ...
                try:
                    ...
                    logger.warning(
                        "⚠️ SemanticMemory PostgreSQL недоступна (попытка %d/%d): %s",
                        self._init_attempts, self._max_init_attempts, e
                    )
            ...
            if self._available is not True:
                ...
                logger.error("❌ SemanticMemory PostgreSQL недоступна после %d попыток", self._max_init_attempts)
    ...
    def add_concept(
        ...
        # Синхронизация с Knowledge Graph (SQLite)
        try:
            ...
            kg.add_concept(
                ...
                metadata={"definition": definition, "domain": domain}
            )
        except Exception as e:
            logger.warning(f"Knowledge Graph sync error (add_concept): {e}")
