        ...
        for phase_name, phase in self._phases:
            ...
            if phase_name in _independent_group:
                ...
                try:
                    ...
                    logger.warning("Timeout: параллельная группа фаз превысила 30с")
                    gathered = [PhaseResult(success=False, errors=["timeout"]) for _ in parallel_set]
            ...
            if phase_name == "generate":
                ...
                cognitive_keys = ["roots_context", "persona_context", "rag_context", "episodic_context", "procedure_context", "graph_context", "emotion_state", "emotion_style", "intent", "strategy"]
