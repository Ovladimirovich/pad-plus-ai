...
class InputSanitizer:
    ...
    # Широкие паттерны вида ;\w+ убраны — дают ложные срабатывания
    # на русской пунктуации (списки через ;) с английскими акронимами (; AI, ; RAG).
    # Реальные атаки покрыты специфичными паттернами ниже:
    # - Подстановки: $(), ` `
    # - Перенаправление на Windows: > C:, >> C:, < C:
    # - Опасные команды: wget, curl, bash, rm -rf и т.д.
    # - Конвейеры с командами: | bash, | sh и т.д. (но не | AI)
    COMMAND_INJECTION_PATTERNS = [
        ...
        r'\|\s*(wget|curl|nc|ncat|bash|sh|cmd|powershell|python|perl|ruby)\b',  # Pipe + опасная команда
        r'\|\|\s*(wget|curl|nc|ncat|bash|sh|cmd|powershell|python|perl|ruby)\b',  # OR + опасная команда
        r';\s*(wget|curl|nc|ncat|bash|sh|cmd|powershell|python|perl|ruby)\b',  # Semicolon + опасная команда
        r'&&\s*(wget|curl|nc|ncat|bash|sh|cmd|powershell|python|perl|ruby)\b',  # AND + опасная команда
        r'(?<!&)\b(wget|curl|nc|ncat|bash|sh|cmd|powershell|python|perl|ruby)\b',  # Отдельная опасная команда
        r'\b(rm|del|format|mkfs)\b\s+[-/]',  # Опасные команды с флагами
    ]
