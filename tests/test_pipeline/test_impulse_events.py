...
@pytest.mark.asyncio
async def test_applies_impulse_deltas():
    mock_core = MagicMock()
    ...
    with patch("scripts.impulse.get_impulse_core", return_value=mock_core), \
         patch("scripts.impulse.get_manager", return_value=mock_manager):
        ...
        assert dim_improve.weight == 0.3 + 0.20 * 0.7
...
async def test_skips_low_significance():
    mock_core = MagicMock()
    with patch("scripts.impulse.get_impulse_core", return_value=mock_core), \
         patch("scripts.impulse.get_manager"):
...
@pytest.mark.asyncio
async def test_skips_unknown_type():
    mock_core = MagicMock()
    with patch("scripts.impulse.get_impulse_core", return_value=mock_core), \
         patch("scripts.impulse.get_manager"):
...
async def test_handles_exception_gracefully():
    with patch("scripts.impulse.get_impulse_core", side_effect=ValueError("fail")), \
